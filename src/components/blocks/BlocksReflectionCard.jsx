...
const NEXT_STEPS = [
  { label: "You've named what's been in the way", active: true },
  { label: 'Rediscover your values, strengths, and ikigai', active: false },
  ...
  return (
    <div className="bg-brand-surface rounded-2xl p-6 border border-brand-border space-y-3">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-xs text-brand-muted">Reading what you shared…</span>
      </div>
      <div className="space-y-2.5 animate-pulse">
        ...
        <div className="h-3.5 bg-brand-border rounded-full w-[75%]" />
        <div className="h-3.5 bg-brand-border rounded-full w-[55%]" />
      </div>
    </div>
  )
}
...
export default function BlocksReflectionCard({ reflection, loading, error, onRetry, onContinue }) {
  return (
    <div className="space-y-4">
      ...
      <div className="text-center pb-2">
        ...
        <h2 className="font-heading font-bold text-2xl text-brand-text">
          Here's what's been in the way.
        </h2>
        <p className="text-brand-muted text-sm mt-2">
          Naming it is the first step past it.
        </p>
      </div>
      ...
      {/* What's next — always visible once loaded */}
      {!loading && (

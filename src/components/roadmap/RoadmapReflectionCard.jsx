...
const NEXT_STEPS = [
  { label: "You've mapped your career path and first move", active: true },
  { label: 'Build your week-by-week action plan', active: false },
  { label: 'Daily tracking and accountability', active: false },
]
  ...
  return (
    <div className="bg-brand-surface rounded-2xl p-6 border border-brand-border space-y-3">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-xs text-brand-muted">Mapping your path…</span>
      </div>
      <div className="space-y-2.5 animate-pulse">
        <div className="h-3.5 bg-brand-border rounded-full w-full" />
        <div className="h-3.5 bg-brand-border rounded-full w-[90%]" />
        <div className="h-3.5 bg-brand-border rounded-full w-[82%]" />
        <div className="h-3.5 bg-brand-border rounded-full w-[95%]" />
        <div className="h-3.5 bg-brand-border rounded-full w-[68%]" />
      </div>
    </div>
  )
}
...
export default function RoadmapReflectionCard({
  reflection,
  loading,
  error,
  onRetry,
    ...
    <div className="space-y-4">
      ...
      {firstStep && (
        <div className="bg-white rounded-2xl p-5 border border-brand-border space-y-3.5">
          ...
          <div>
            <p className="text-[10px] text-brand-muted mb-1.5">The step this week</p>
          ...
          {firstMoveBlocker && (
            <>
              ...
              <div>
                <p className="text-[10px] text-brand-muted mb-1.5">What's been stopping you</p>
                <p className="text-sm text-brand-text italic leading-relaxed">
                  &ldquo;{firstMoveBlocker}&rdquo;
                </p>
              </div>
            </>
          )}
